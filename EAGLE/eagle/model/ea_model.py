...
class EaModel(nn.Module):
    ...
    def __init__(
        ...
        if load_.missing_keys or load_.unexpected_keys:
            print(
                "EAGLE draft load_state_dict used strict=False; "
                f"missing_keys={load_.missing_keys}, unexpected_keys={load_.unexpected_keys}"
            )
        if self.use_eagle3 and config.vocab_size != config.draft_vocab_size:
            if (
                not hasattr(self.ea_layer, "t2d")
                or self.ea_layer.t2d.sum().item() != config.draft_vocab_size
                or not hasattr(self.ea_layer, "d2t")
                or self.ea_layer.d2t.numel() != config.draft_vocab_size
            ):
                raise RuntimeError(
                    "Invalid or missing EAGLE3 draft-vocab mapping in checkpoint. "
                    "Expected t2d/d2t buffers with draft_vocab_size entries."
                )
